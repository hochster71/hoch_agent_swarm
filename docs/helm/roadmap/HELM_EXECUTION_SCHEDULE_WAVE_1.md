...
`BACKLOG -> ASSIGNED -> BUILT -> TESTED -> LOCAL_VERIFIED -> INTEGRATION_PENDING -> INTEGRATED -> LIVE_VERIFIED`

No agent may claim plain `COMPLETE` without scope-qualified evidence.

### LOCAL_VERIFIED vs INTEGRATED (founder rule, 2026-07-20)

A commit that exists only in a working copy is **not** integrated. Evidence must state
which side of the repository of record it sits on:

| State | Means | Proof |
|---|---|---|
| `AMBIGUOUS_SHA` | SHA shorter than 40 chars — verdict refused | supply the full SHA |
| `UNKNOWN` | commit not found in any ref | cannot verify; do not report progress |
| `LOCAL_VERIFIED` | exists locally, on NO remote | commit SHA + test output |
| `INTEGRATED_ELSEWHERE` | on a remote, but not the target branch | naming the branch it IS on |
| `INTEGRATED` | reachable from the target remote branch | `git branch -r --contains <full-sha>` |

Enforced in code: `backend/helm_runtime.mission_envelope.integration_state()`.

Two rules, both learned by getting them wrong on 2026-07-20:

**Always use the full 40-character SHA.** Git resolves any unique prefix, so an
abbreviated SHA can confirm delivery of a commit nobody wrote. Two Phase D SHAs cited
that day both began `5877ffa1` — one real, one mistyped. Querying the 8-char prefix
returned `INTEGRATED`. A verification tool produced the false green it existed to
prevent. Abbreviated input now returns `AMBIGUOUS_SHA` instead of a verdict.

**Absent-from-target is not absent-from-everywhere.** A branch-scoped query that finds
nothing was first reported as `LOCAL_VERIFIED`, which reads as "never pushed." Two
redactor commits were reported that way while sitting on `helm-runtime-bridge-v1` the
whole time — leading to a wrong instruction to push work already delivered.
`INTEGRATED_ELSEWHERE` now separates the two.

Local test success is real evidence of correctness and no evidence of delivery. A
delivery check that mislabels local as remote is the same failure mode as a redactor
that mislabels secret as safe: the checker needs checking.
...
- **`scripts/kimi/**` — the handoff redactor that decides what may leave the repo for an
  external agent. Per-action founder approval; never always-approve while editing it.**
