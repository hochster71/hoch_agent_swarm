...
and 30-day elapsed time enforcement rules (PROD-001 through PROD-012).
"""
...
def test_prod_001_burnin_manifest_generation():
    ...
    assert len(m["qualified_source_commit"]) > 0
...
def test_prod_003_zero_replay_divergence_verification():
    ...
    assert obs["persisted_decision_code"] == "APPROVED"
...
def test_prod_007_mutated_record_content_invalidates_hash():
...
def test_prod_011_algorithmic_gap_accounting_detection(tmp_path):
    """[PROD-011] Asserts missing interval gap greater than expected interval is algorithmically calculated."""
    ledger_file = tmp_path / "gap_ledger.jsonl"

    obs1 = {"sequence_number": 1, "previous_hash": GENESIS_HASH, "timestamp_utc": "2026-07-22T00:00:00Z"}
    obs1["record_hash"] = recompute_record_hash(GENESIS_HASH, obs1)

    # 15 minutes gap (300 sec interval -> 2 skipped intervals)
    obs2 = {"sequence_number": 2, "previous_hash": obs1["record_hash"], "timestamp_utc": "2026-07-22T00:15:00Z"}
    obs2["record_hash"] = recompute_record_hash(obs1["record_hash"], obs2)
