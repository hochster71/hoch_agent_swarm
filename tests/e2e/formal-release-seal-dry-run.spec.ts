...
test.describe("@legacy @compat @deorbited Formal Release Seal Dry Run E2E", () => {
  test("runs preview, requests approval, approves preview, and executes seal dry run successfully", async ({ page }) => {
    ...
    await versionInput.fill("0.1.7-candidate.3");
    ...
    await reasonInput.fill("Create candidate packet for seal dry run E2E test");
    ...
    page.on("dialog", async dialog => {
      ...
      if (dialog.type() === "prompt") {
        await dialog.accept("Simulated formal release approval for seal dry run E2E testing");
      } else {
        await dialog.accept();
      }
    });
    ...
    await previewReasonInput.fill("Preview formal readiness for seal dry run E2E test");
    ...
    // 8. Now select this approved preview in the Seal Dry Run panel
    const dryRunSelect = page.locator("#seal-dry-run-preview-select");
    await expect(dryRunSelect).toBeVisible();
    // Wait for dropdown option to populate (the app refreshes dropdowns after approval)
    ...
    // 9. Click Execute Seal Dry Run button
    ...
    // 10. Wait for seal dry run output elements
    ...
    expect(dryRunId?.startsWith("seal-dry-")).toBeTruthy();

    // 11. Assert files are generated on disk
    const workspaceRoot = "/Users/michaelhoch/.gemini/antigravity/scratch/hoch-agent-swarm";
    const dryRunJsonPath = path.join(workspaceRoot, "dist/formal-previews", previewId!, "formal_release_seal_dry_run_manifest.json");
    const dryRunMdPath = path.join(workspaceRoot, "dist/formal-previews", previewId!, "formal_release_seal_dry_run_report.md");

    expect(fs.existsSync(dryRunJsonPath)).toBeTruthy();
    expect(fs.existsSync(dryRunMdPath)).toBeTruthy();

    const dryRunContent = JSON.parse(fs.readFileSync(dryRunJsonPath, "utf8"));
    expect(dryRunContent.seal_dry_run_id).toBe(dryRunId);
    expect(dryRunContent.seal_status).toBe(dryRunStatus);

    // 12. Assert no git tags were created
    const tagMatch = fs.existsSync(path.join(workspaceRoot, ".git/refs/tags/v0.1.7-candidate.3"));
    expect(tagMatch).toBeFalsy();

    // Take screenshot
    await page.waitForTimeout(1000);
    const screenshotPath = "artifacts/qa/formal-release-seal-dry-run.png";
