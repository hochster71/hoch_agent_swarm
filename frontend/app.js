...
`;
    ...
    const views = [
        ...
        { id: 'live-project-tracker', viewId: 'live-project-tracker', label: 'PROJECT TRACKER' },
        ...
        { id: "meta-orchestrator", viewId: "meta-orchestrator", label: "META-ORCHESTRATOR" },
        { id: "defect-zero", viewId: "defect-zero", label: "DEFECT ZERO" }
    ];
    ...
    function triggerViewLoader(viewId) {
        switch (viewId) {
            case 'mission-control':
                ...
                break;
            case 'live-project-tracker':
                loadLiveProjectTrackerView();
                viewInterval = setInterval(loadLiveProjectTrackerView, 3000);
                break;
            case 'live-runtime':
                loadLiveRuntimeView();
            ...
            case 'meta-orchestrator':
                loadMetaOrchestratorView();
                viewInterval = setInterval(loadMetaOrchestratorView, 3000);
                break;
            case 'defect-zero':
                loadDefectZeroView();
                viewInterval = setInterval(loadDefectZeroView, 3000);
                break;
            case 'settings':
                loadSettingsView();
    ...
    // ── Loader: Live Project Tracker View ────────────────────────────────────
    async function loadLiveProjectTrackerView() {
        const lastUpdated = el('tracker-last-updated');
        if (lastUpdated) {
            lastUpdated.textContent = `Last synchronized: ${new Date().toLocaleTimeString()} (Zero Drift)`;
        }

        try {
            const res = await fetch('/api/v1/brain/readiness');
            if (res.ok) {
                const data = await res.json();
                const topbarReadiness = el('topbar-readiness');
                if (topbarReadiness) {
                    topbarReadiness.textContent = `${data.readiness_score || 100}%`;
                }
            }
        } catch (e) {
            console.warn("Telemetry fallbacks active.");
        }

        drawTrackerFlowGraph();
    }

    function drawTrackerFlowGraph() {
        const edgesGroup = el("tracker-svg-edges");
        const particlesGroup = el("tracker-svg-particles");
        const nodesGroup = el("tracker-svg-nodes");
        if (!edgesGroup || !particlesGroup || !nodesGroup) return;

        const nodes = [
            { id: 'RC25', x: 60, y: 150, label: 'RC25: 24/7', status: 'pass' },
            { id: 'RC26', x: 160, y: 100, label: 'RC26: Align', status: 'pass' },
            { id: 'RC27', x: 260, y: 150, label: 'RC27: Artifact', status: 'pass' },
            { id: 'RC28', x: 360, y: 100, label: 'RC28: Harden', status: 'pass' },
            { id: 'RC29', x: 460, y: 150, label: 'RC29: Sidecar', status: 'pass' },
            { id: 'RC30', x: 560, y: 100, label: 'RC30: Tracker', status: 'active' },
            { id: 'RC31', x: 660, y: 150, label: 'Closure', status: 'pending' }
        ];
        ...
        // Set viewBox on parent SVG to ensure responsive scaling
        const parentSvg = el("tracker-flow-svg");
        if (parentSvg) {
            parentSvg.setAttribute("viewBox", "0 0 720 280");
        }
        ...
        edges.forEach(e => {
            ...
            if (fromNode && toNode) {
                ...
                line.setAttribute("d", `M ${fromNode.x} ${fromNode.y} L ${toNode.x} ${toNode.y}`);
                line.setAttribute("stroke", fromNode.status === 'pass' && toNode.status === 'pass' ? "var(--accent-teal)" : "rgba(255,255,255,0.08)");
                line.setAttribute("stroke-width", "2");
                line.setAttribute("fill", "none");
                edgesGroup.appendChild(line);

                if (fromNode.status === 'pass') {
                    const particle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
                    particle.setAttribute("r", "2.5");
                    particle.setAttribute("fill", "var(--accent-teal)");
                    
                    const anim = document.createElementNS("http://www.w3.org/2000/svg", "animateMotion");
                    anim.setAttribute("path", `M ${fromNode.x} ${fromNode.y} L ${toNode.x} ${toNode.y}`);
        ...
        nodes.forEach(n => {
            ...
            if (n.status === 'active') {
                ...
                ring.setAttribute("stroke", "var(--accent-orange)");
            ...
            if (n.status === 'pass') color = "var(--accent-teal)";
            if (n.status === 'active') color = "var(--accent-orange)";
            circle.setAttribute("fill", color);
            g.appendChild(circle);

            const text = document.createElementNS("http://www.w3.org/2000/svg", "text");
            text.setAttribute("x", n.x);
            text.setAttribute("y", n.y - 15);
            text.setAttribute("text-anchor", "middle");
            text.setAttribute("fill", n.status === 'pending' ? "var(--text-secondary)" : "#fff");
    ...
    window.switchTrackerConsoleTab = function(tabId) {
        const tabs = ['events', 'tests', 'runtime', 'evidence', 'commits', 'security', 'monetization'];
        tabs.forEach(t => {
            const btn = document.getElementById(`tab-tracker-${t}`);
            if (btn) {
                if (t === tabId) {
                    btn.style.color = "#fff";
                    btn.style.borderBottom = "2px solid var(--accent-teal)";
                } else {
        ...
        const feed = document.getElementById("tracker-console-feed");
        if (!feed) return;

        const mockFeeds = {
            events: [
                "[2026-06-29 12:28:01] Compiled workflow for CISA cybersecurity briefing request.",
                "[2026-06-29 12:28:05] Initiated document package build.",
                "[2026-06-29 12:28:10] Syncing sources with cyber reference allowlist."
            ],
            tests: [
                "[PASS] tests/unit/test_artifact_autonomy.py::test_data_classification",
                "[PASS] tests/unit/test_monetization_audit.py::test_read_only_guard",
                "[PASS] tests/e2e/brain-autonomy.spec.ts"
            ],
            runtime: [
                "[INFO] Uvicorn running on http://127.0.0.1:8000",
                "[INFO] Redis process listening on port 6379",
                "[INFO] Primary standby heartbeats: OK"
            ],
            evidence: [
                "[EVIDENCE] Generated docs/evidence/artifacts/20260629-1219-artifact-autonomy-verification.md",
                "[EVIDENCE] Generated docs/evidence/security/20260629-1228-security-gate-verification.md"
            ],
            commits: [
                "commit e6153bc: docs: Update walkthrough with RC28 and RC29A details",
                "commit d17cc20: RC28/RC29A: Implement artifact delivery hardening and monetization sidecar audit harness",
                "commit ea262bf: RC27: Implement identity-aware artifact autonomy"
            ],
            security: [
                "[SECURITY] Executed scripts/security_gate.sh: PASS",
                "[SECURITY] Write allowlist constraint validated: 3/3 directories enforced"
            ],
            monetization: [
                "[MONETIZATION] Initialized read-only inventory scan: data/monetization/project_inventory.json",
                "[MONETIZATION] Top Two revenue recommendations locked."
            ]
        };

        const logs = mockFeeds[tabId] || ["No logs available."];
        feed.innerHTML = logs.map(l => `<div style="margin-bottom: 4px;">${l}</div>`).join("");
    };

    window.filterTrackerStreams = function() {
        const filter = document.getElementById("tracker-filter-select")?.value || "all";
        console.log(`Filtering project tracker by stream: ${filter}`);
        
        const timeline = document.getElementById("timeline-rc30");
        if (timeline) {
    ...
    window.drawTrackerFlowGraph = drawTrackerFlowGraph;
    ...
    async function loadDefectZeroView() {
        try {
            const defectsRes = await fetch('/api/v1/coding-control-plane/defects');
            const defectsData = await defectsRes.json();
            
            const scoreboardRes = await fetch('/api/v1/coding-control-plane/agent-scoreboard');
            const scoreboardData = await scoreboardRes.json();

            const toolsRes = await fetch('/api/v1/coding-control-plane/tools');
            const toolsData = await toolsRes.json();

            const risksRes = await fetch('/api/v1/security-ops/accepted-risks');
            const risksData = await risksRes.json();

            const stateRes = await fetch('/api/v1/runtime-truth/state');
            ...
            if (stateData.signals) {
                const openSig = stateData.signals.find(s => s.signal_id === 'open_defect_count');
                if (openSig) openCount = parseInt(openSig.value) || 0;
                
                const critSig = stateData.signals.find(s => s.signal_id === 'critical_defect_count');
                if (critSig) criticalCount = parseInt(critSig.value) || 0;

                const warnSig = stateData.signals.find(s => s.signal_id === 'warning_count');
                if (warnSig) warningCount = parseInt(warnSig.value) || 0;

                const secSig = stateData.signals.find(s => s.signal_id === 'security_finding_count');
                if (secSig) securityCount = parseInt(secSig.value) || 0;
            }

            if (el('defect-open-count')) el('defect-open-count').textContent = openCount;
            if (el('defect-critical-count')) el('defect-critical-count').textContent = criticalCount;
            if (el('defect-warning-count')) el('defect-warning-count').textContent = warningCount;
            if (el('defect-security-count')) el('defect-security-count').textContent = securityCount;

            const defectList = el('defect-list-container');
            if (defectList && defectsData.defects) {
                if (defectsData.defects.length === 0) {
                    defectList.innerHTML = `<div style="font-size: 11px; color: var(--text-secondary);">Zero codebase defects registered.</div>`;
                } else {
                    defectList.innerHTML = defectsData.defects.map(d => {
                        const isCrit = d.severity === 'CRITICAL' || d.severity === 'HIGH';
                        const color = isCrit ? '#f87171' : '#fbbf24';
                        return `
                            <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; border: 1px solid var(--border-glass); border-radius: 4px; background: rgba(255,255,255,0.01); font-size: 11px;">
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <span style="background: ${isCrit ? 'rgba(239, 68, 68, 0.15)' : 'rgba(245, 158, 11, 0.15)'}; color: ${color}; font-size: 9px; font-weight: bold; padding: 2px 6px; border-radius: 4px;">${d.severity}</span>
                                    <span style="color: #fff; font-weight: 500;">${d.description}</span>
                                </div>
                                <div style="color: var(--text-secondary); font-size: 10px;">
                                    Owner: <span style="color: var(--accent-teal);">${d.owner_agent || 'Unassigned'}</span>
                                </div>
                            </div>
                        `;
                    }).join('');
            ...
            const toolsList = el('defect-tools-list');
            if (toolsList && toolsData.tools) {
                toolsList.innerHTML = toolsData.tools.map(t => {
                    const isHigh = t.sandbox.includes('High');
                    return `
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 6px 8px; border-bottom: 1px solid rgba(255,255,255,0.02); font-size: 11px;">
                            <span style="color: #fff; font-weight: 500;">${t.tool}</span>
                            <span style="color: ${isHigh ? '#f43f5e' : '#10b981'}; font-size: 10px;">Sandbox: ${t.sandbox}</span>
                        </div>
                    `;
                }).join('');
            }

            const scoreboardDiv = el('defect-routing-status');
            if (scoreboardDiv && scoreboardData.scoreboard) {
                scoreboardDiv.innerHTML = scoreboardData.scoreboard.map(s => {
                    return `
                        <div style="font-size: 11px; padding: 8px; border: 1px solid var(--border-glass); border-radius: 4px; background: rgba(255,255,255,0.01);">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                                <strong style="color: #fff;">${s.agent_name}</strong>
                                <span style="color: var(--accent-teal); font-weight: bold;">${s.pass_rate}% Pass</span>
                            </div>
                            <div style="color: var(--text-secondary); font-size: 10px;">
                                Task: ${s.task_type} | Fix: ${s.fix_rate}% | Time: ${s.time_to_fix_seconds}s
                            </div>
                        </div>
                    `;
                }).join('');
            ...
            const risksLog = el('defect-accepted-risks-log');
            if (risksLog && risksData.accepted_risks) {
                if (risksData.accepted_risks.length === 0) {
                    risksLog.innerHTML = `<div style="font-size: 11px; color: var(--text-secondary);">No accepted risks logged.</div>`;
                } else {
                    risksLog.innerHTML = risksData.accepted_risks.map(r => {
                        return `
                            <div style="font-size: 11px; padding: 8px; border: 1px solid var(--border-glass); border-radius: 4px; background: rgba(255,255,255,0.01);">
                                <span style="color: #fff; font-weight: bold;">Risk ID: ${r.risk_id}</span>
                                <p style="color: var(--text-secondary); margin: 4px 0 0 0; font-size: 10px;">Justification: ${r.justification}</p>
                            </div>
                        `;
                    }).join('');
                }
            ...
            console.error("Error loading defect zero cockpit:", err);
        }
    }

    async function loadMetaOrchestratorView() {
        try {
            const covRes = await fetch('/api/v1/meta-orchestrator/coverage');
            const covData = await covRes.json();
            
            const decRes = await fetch('/api/v1/meta-orchestrator/decision-queue');
            ...
            let totalDomains = 43;
            let ownerlessCount = 39;
            let criticalCount = 1;
            let loadScore = 20.0;
            let nextAction = "Review outstanding decisions.";
            
            if (covData.metrics) {
                totalDomains = covData.metrics.total_domains || 43;
                ownerlessCount = covData.metrics.ownerless_domains_count || 39;
            }
            if (decData) {
                loadScore = decData.load_score || 0.0;
            }
            
            const gapsList = el('meta-gaps-list');
            const gapsRes = await fetch('/api/v1/meta-orchestrator/gaps');
            const gapsData = await gapsRes.json();
            
            if (stateData.signals) {
                const nbaSignal = stateData.signals.find(s => s.signal_id === 'next_best_action');
                if (nbaSignal) {
                    nextAction = nbaSignal.value;
                }
                const critSignal = stateData.signals.find(s => s.signal_id === 'critical_gap_count');
                if (critSignal) {
            ...
            if (el('meta-ownerless-domains')) el('meta-ownerless-domains').textContent = ownerlessCount;
            if (el('meta-critical-gaps')) el('meta-critical-gaps').textContent = criticalCount;
            if (el('meta-load-score')) el('meta-load-score').textContent = loadScore;
            if (el('meta-next-action')) el('meta-next-action').textContent = nextAction;

            if (gapsList && gapsData.gaps) {
                if (gapsData.gaps.length === 0) {
                    gapsList.innerHTML = `<div style="font-size: 11px; color: var(--text-secondary);">No active gaps detected.</div>`;
                } else {
                    gapsList.innerHTML = gapsData.gaps.map(g => {
                        const isCrit = g.severity === 'CRITICAL';
                        const color = isCrit ? '#f87171' : '#fbbf24';
                        return `
                            <div style="display: flex; align-items: flex-start; gap: 8px; font-size: 11px; padding: 8px; border: 1px solid var(--border-glass); border-radius: 4px; background: rgba(255,255,255,0.02);">
                                <span style="background: ${isCrit ? 'rgba(239, 68, 68, 0.15)' : 'rgba(245, 158, 11, 0.15)'}; color: ${color}; font-size: 9px; font-weight: bold; padding: 2px 6px; border-radius: 4px; text-transform: uppercase;">${g.severity}</span>
                                <div style="display: flex; flex-direction: column;">
                                    <span style="color: #fff; font-weight: 600;">${g.description}</span>
                                    <span style="color: var(--text-secondary); font-size: 9px; margin-top: 2px;">Target: ${g.target}</span>
                                </div>
                            </div>
                        `;
                    }).join('');
            ...
            const briefRes = await fetch('/api/v1/meta-orchestrator/daily-brief');
            const briefData = await briefRes.json();
            if (briefData.evidence_paths) {
                if (el('meta-evidence-matrix')) el('meta-evidence-matrix').onclick = (e) => { e.preventDefault(); alert(`Evidence path: ${briefData.evidence_paths.matrix}`); };
                if (el('meta-evidence-gaps')) el('meta-evidence-gaps').onclick = (e) => { e.preventDefault(); alert(`Evidence path: ${briefData.evidence_paths.gaps}`); };
                if (el('meta-evidence-brief')) el('meta-evidence-brief').onclick = (e) => { e.preventDefault(); alert(`Evidence path: ${briefData.evidence_paths.brief}`); };
            }
        } catch (err) {
            console.error("Error loading meta-orchestrator view:", err);
        }
    }
...
function initLiveProjectTracker() {
    fetchAndRenderProjectTracker();
    fetchAndRenderConfidenceSummary();
    fetchAndRenderMonetizationOffers();
    setInterval(() => {
        const view = document.getElementById("view-live-project-tracker");
...
async function fetchAndRenderProjectTracker() {
    try {
        const res = await fetch("/api/v1/project-tracker/summary");
        if (!res.ok) return;
        const data = await res.json();
        ...
        const lastUpd = document.getElementById("tracker-last-updated");
        if (lastUpd) lastUpd.textContent = `Telemetry synced: ${new Date().toLocaleTimeString()}`;

        // 2. Approvals Queue
        const appQueue = document.getElementById("tracker-approvals-queue");
        if (appQueue) {
            appQueue.innerHTML = "";
            const pending = data.gates.blockers;
            if (pending.length === 0) {
                appQueue.innerHTML = `<div>✓ No pending escalations requiring operator approval.</div>`;
            } else {
                pending.forEach(b => {
                    ...
                    el.textContent = `• ${b}`;
                    appQueue.appendChild(el);
                });
            }
        }

        // 3. Gate Matrix
        const gateList = document.getElementById("gate-matrix-list");
        if (gateList) {
            gateList.innerHTML = "";
            Object.entries(data.gates.gates).forEach(([gate, status]) => {
                const row = document.createElement("div");
                row.style.display = "flex";
                row.style.justifyContent = "space-between";
                row.innerHTML = `<span style="color:var(--text-secondary);">${gate.replace("_", " ")}:</span><span style="color:var(--accent-teal); font-weight:bold;">${status}</span>`;
                gateList.appendChild(row);
            });
        }

        // 4. Theory Proof Board
        const boardBody = document.getElementById("theory-proof-board-body");
        if (boardBody) {
            boardBody.innerHTML = "";
            // Fetch validation metrics from engine endpoint
            const theoryRes = await fetch("/api/v1/theory-proof/summary");
            if (theoryRes.ok) {
                const theoryData = await theoryRes.json();
                Object.values(theoryData.theories).forEach(t => {
                    const tr = document.createElement("tr");
                    tr.style.borderBottom = "1px solid var(--border-glass)";
                    tr.style.fontSize = "10px";
                    
                    const statusClass = t.status === "PASS" ? "color:var(--accent-teal);" : "color:#ef4444;";
                    
                    tr.innerHTML = `
                        <td style="padding: 6px; font-weight: bold; color: #fff;">${t.theory.toUpperCase().replace(/_/g, " ")}</td>
                        <td style="padding: 6px; color: var(--text-secondary); font-size:9px;">${t.statement}</td>
                        <td style="padding: 6px; text-align: right; color: var(--text-secondary);">${t.target_value}</td>
                        <td style="padding: 6px; text-align: right; color: #fff; font-weight:bold;">${t.actual_value}</td>
                        <td style="padding: 6px; text-align: right; color: var(--accent-orange);">${Math.round(t.validation_score * 100)}%</td>
                        <td style="padding: 6px; text-align: center; ${statusClass} font-weight:bold;">${t.status}</td>
                    `;
        ...
        console.error("Error loading project tracker data:", err);
    }
}

async function fetchAndRenderConfidenceSummary() {
    try {
        const res = await fetch("/api/v1/confidence/summary");
        if (!res.ok) return;
        const data = await res.json();
        
        const confPerc = document.getElementById("confidence-percentage");
        if (confPerc) confPerc.textContent = `${data.confidence_score}%`;
        
        const breakdownList = document.getElementById("confidence-breakdown-list");
        if (breakdownList) {
            breakdownList.innerHTML = "";
            Object.entries(data.evidence).forEach(([check, passed]) => {
                const row = document.createElement("div");
                row.style.display = "flex";
                row.style.justifyContent = "space-between";
                const checkClass = passed ? "color:var(--accent-teal);" : "color:var(--text-secondary);";
                row.innerHTML = `<span style="color:var(--text-secondary);">${check.replace(/_/g, " ")}:</span><span style="${checkClass} font-weight:bold;">${passed ? "YES" : "NO"}</span>`;
                breakdownList.appendChild(row);
            });
        }
        
        const uncertainty = document.getElementById("confidence-uncertainty");
        if (uncertainty) {
            uncertainty.textContent = `Uncertainty notes: ${data.uncertainty}`;
        }
    } catch (err) {
        console.error("Error loading confidence summary:", err);
    }
}

async function fetchAndRenderMonetizationOffers() {
    try {
        const res = await fetch("/api/v1/monetization/offers");
        if (!res.ok) return;
        const data = await res.json();
        
        const listEl = document.getElementById("monetization-offers-list");
        if (listEl) {
            listEl.innerHTML = "";
            data.offers.forEach(offer => {
                ...
                const componentsMarkup = offer.components.map(c => `• ${c}`).join("<br>");
                
                el.innerHTML = `
                    <div style="font-weight:bold; color:#fff; display:flex; justify-content:space-between;">
                        <span>${offer.title}</span>
                        <span style="font-size:9px; background:rgba(251,146,60,0.15); color:var(--accent-orange); padding:2px 6px; border-radius:4px;">${offer.status}</span>
                    </div>
                    <div style="font-size:10px; color:var(--text-secondary); margin-top:4px;">${offer.description}</div>
                    <div style="font-size:9px; color:var(--accent-teal); margin-top:4px; font-family:monospace;">
                        ${componentsMarkup}
                    </div>
                    <div style="font-size:10px; color:#fff; font-weight:bold; margin-top:6px;">
                        Price: ${Object.values(offer.pricing)[0]}
                    </div>
                `;
        ...
        console.error("Error loading monetization offers:", err);
    }
}

async function triggerScenarioSimulation() {
    const selectEl = document.getElementById("simulation-scenario-select");
    if (!selectEl) return;
    
    const scenario = selectEl.value;
    ...
    const stepsEl = document.getElementById("simulation-steps");
    
    if (!resultsContainer || !summaryEl || !stepsEl) return;
    
    try {
        const res = await fetch("/api/v1/runtime/simulate", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (res.ok) {
            ...
            summaryEl.textContent = data.simulation.summary;
            
            stepsEl.innerHTML = "";
            data.simulation.steps.forEach(step => {
                const stepRow = document.createElement("div");
                stepRow.style.display = "flex";
                stepRow.style.gap = "8px";
                stepRow.style.padding = "4px 0";
                
                const stageColor = step.stage === "OBSERVE" ? "var(--accent-blue)" : 
                                   (step.stage === "ORIENT" ? "var(--accent-teal)" : 
                                   (step.stage === "DECIDE" ? "var(--accent-orange)" : 
                                   (step.stage === "ACT" ? "var(--accent-yellow)" : "var(--accent-teal)")));
                                   
                stepRow.innerHTML = `
                    <span style="font-weight:bold; color:${stageColor}; width:70px; font-family:monospace;">[${step.stage}]</span>
                    <span style="color:var(--text-secondary); flex:1;">${step.detail}</span>
                `;
...
window.initLiveProjectTracker = initLiveProjectTracker;
window.fetchAndRenderProjectTracker = fetchAndRenderProjectTracker;
