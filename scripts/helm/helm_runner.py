...
class HELMMissionRunner:
    ...
    def verify_everything(self):
        ...
        # 2. Check HELM worktree cleanliness for governed runner and manifest files
        cmd_helm_status = subprocess.run(["git", "status", "--porcelain", "--", "scripts/helm/helm_runner.py", "coordination/evidence/build_12/gate_2_purchase/gate_2_evidence_manifest.json"], capture_output=True, text=True, cwd=self.workspace_root)
