...
# =============================================================================
# qa_product_gate.sh
# Verification gate for the product team dossier.
# =============================================================================
set -euo pipefail

DOSSIER_FILE="data/qa_dossiers/product_qa.json"

echo "==> Probing product QA dossier..."
...
if [ "$STATUS" != "PASS" ]; then
  echo "❌ FAIL: product QA Status is '$STATUS'"
  echo "Unresolved defects:"
  jq -r '.unresolved_defects[]' "$DOSSIER_FILE" 2>/dev/null || echo "None"
  exit 1
fi

echo "✅ product QA Dossier Verification Gate Passed."
exit 0
