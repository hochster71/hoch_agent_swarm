...
GIT_DIRTY=$(git status --short | grep -v -E "rc32|pert_server|goal_completion_contract|has_autonomous_cadence|has_parallel_mirror_verify|logs/|pert_command_metrics|rc29_release_verify" || true)
if [ -z "${GIT_DIRTY}" ]; then
