// engine/index.js
//
// HFF — Vendor Spend Rollup — engine entry point.
//
// buildReport(csvText, opts) runs the whole deterministic pipeline:
//   validate -> ingest (flag bad rows) -> spend analysis -> summarize ->
...
const { analyzeSpend } = require('./spend');
...
function analyze(csvText, opts) {
  ...
  const analysis = analyzeSpend(ingested.payments, {
    hasCategory: ingested.hasCategory,
    vendorFromDescription: ingested.vendorFromDescription,
    amountMode: ingested.amountMode,
