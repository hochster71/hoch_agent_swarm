...
//
// HFF — Getting-Paid Speed Report — PAID report endpoint. Returns the full
// report JSON plus the XLSX workbook and one-page PDF as base64.
//
...
//   3. DEV UNLOCK — PAYTIMING_DEV_UNLOCK=1 (local/dev only). NOT real payment.
...
async function isEntitled(req, body) {
  const devUnlock = process.env.PAYTIMING_DEV_UNLOCK === '1';
  ...
  return {
    ok: false,
    code: 501,
    message: 'Report generation is not configured. Set STRIPE_SECRET_KEY (buyers gate on session_id), or set PAYTIMING_DEV_UNLOCK=1 for local testing. See README.md.',
  };
}
  ...
  try {
    ...
    return res.status(200).json({
      ...
      report: {
        ...
        dueBasisMix: report.analysis.dueBasisMix,
      ...
      files: {
        xlsx: { filename: 'getting-paid-speed-report.xlsx', base64: xlsx.toString('base64'), bytes: xlsx.length },
        pdf: { filename: 'getting-paid-speed-summary.pdf', base64: pdf.toString('base64'), bytes: pdf.length },
      },
    });
  } catch (err) {
