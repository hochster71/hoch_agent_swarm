...
//
// HCF — Link & QR Safety Report — Stripe Checkout creator.
//
// One plan:
//   - "report" -> $5 ONE-TIME payment (mode: 'payment')
//
// SAFETY / DESIGN NOTES (mirrors the proven hff-runway shape):
...
//   * Price ID from env (STRIPE_PRICE_REPORT).
...
module.exports = async function handler(req, res) {
  ...
  if (!Object.prototype.hasOwnProperty.call(TIERS, tier)) {
    return res.status(400).json({ error: 'invalid_tier', message: "Body must include a valid 'tier'. Allowed: 'report'." });
  }
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { product: 'hcf-phish-checker', tier },
    });
