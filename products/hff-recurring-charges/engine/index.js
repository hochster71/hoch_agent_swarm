// engine/index.js
//
// HFF — Recurring Charge Finder — engine entry point.
//
// buildReport(csvText, opts) runs the whole deterministic pipeline:
//   validate -> ingest (flag bad rows) -> detect recurrence -> summarize ->
...
function analyze(csvText, opts) {
  ...
  const detection = detectRecurring(ingested.transactions, options);
  const summary = buildSummary(detection, { skippedCount: ingested.skipped.length });

  // HARD GUARDRAIL — fails closed before anything is rendered.
  assertNoAdvice(summaryStrings(summary, detection));
