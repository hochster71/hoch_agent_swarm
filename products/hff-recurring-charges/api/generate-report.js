...
//
// HFF — Recurring Charge Finder — PAID report endpoint. Returns the full report
...
//   3. DEV UNLOCK — RECURRING_DEV_UNLOCK=1 (local/dev only). NOT real payment.
...
// are produced. A violation surfaces as 422 — never a fake success.
'use strict';
...
async function isEntitled(req, body) {
  const devUnlock = process.env.RECURRING_DEV_UNLOCK === '1';
  ...
  return {
    ok: false,
    code: 501,
    message: 'Report generation is not configured. Set STRIPE_SECRET_KEY (buyers gate on session_id), or set RECURRING_DEV_UNLOCK=1 for local testing. See README.md.',
  };
}
  ...
  try {
    ...
    return res.status(200).json({
      ...
      report: {
        ...
        recurring: report.detection.recurring,
        oneOff: report.detection.oneOff,
        overlaps: report.detection.overlaps,
        skipped: report.skipped,
        dateOrder: report.dateOrder,
        disclaimer: report.disclaimer,
      },
      files: {
        xlsx: { filename: 'recurring-charges.xlsx', base64: xlsx.toString('base64'), bytes: xlsx.length },
        pdf: { filename: 'recurring-charges-summary.pdf', base64: pdf.toString('base64'), bytes: pdf.length },
      },
    });
  } catch (err) {
