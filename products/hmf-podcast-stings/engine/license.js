...
function renderLicense(pack, opts) {
  ...
  lines.push('HOCH MUSIC FACTORY (HMF) — PODCAST STING USAGE LICENSE');
  ...
  lines.push('TERM');
  lines.push('----');
  lines.push('  ' + (lic.term || 'Perpetual for end-products you create with these stings.'));
  lines.push('');
  lines.push('GUARDRAIL / WARRANTY');
  lines.push('--------------------');
  lines.push(wrap(
    'Every sting in this pack is an original INSTRUMENTAL work: no vocals, no lyrics, ' +
    'and no artist- or voice-likeness of any real person. Stings are delivered only ' +
    'after this license gate passes.'
  ));
  lines.push('');
  lines.push('STINGS COVERED');
  lines.push('--------------');
  for (const t of pack.tracks) {
    lines.push(`  - ${t.id}  ${t.title}  (${t.type || 'sting'}, ${t.duration_sec}s, ${t.bpm || '?'}bpm, ${t.key || '?'})`);
  }
  lines.push('');
