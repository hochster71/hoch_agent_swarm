...
//
// HFF — Invoice Aging Snapshot — Stripe Checkout creator.
//
// One plan:
//   - "report" -> $9 ONE-TIME payment (mode: 'payment')
//
// SAFETY / DESIGN NOTES (mirrors the proven Runway / Story Studio shape):
...
//   * Price ID from env (STRIPE_PRICE_REPORT) — never assumes a Stripe account.
...
module.exports = async function handler(req, res) {
  ...
  const TIERS = {
    report: { mode: 'payment', priceEnv: 'STRIPE_PRICE_REPORT' },
  };

  if (!Object.prototype.hasOwnProperty.call(TIERS, tier)) {
    return res.status(400).json({
      error: 'invalid_tier',
      message: "Body must include a valid 'tier'. Allowed values: 'report'.",
    });
  }
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { product: 'hff-invoice-aging', tier },
    });
