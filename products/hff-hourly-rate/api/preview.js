...
// uploaded CSV and returns the top-line hour and rate figures plus the first
// three client labels, so a stranger can see the tool actually works before
// paying.
//
// What it withholds (the paid part): the full per-client table, per-client rate
// and session detail, the monthly and weekday series, per-entry rows,
    ...
    return res.status(200).json({
      ...
      topClients: report.analysis.clients
        ...
        .map((c) => ({
          label: c.label,
          hours: c.hours,
          hoursSharePct: c.hoursSharePct,
          entryCount: c.entryCount,
        })),
      withheld:
        'The full per-client table (rates, sessions, billable split per client), the monthly and weekday ' +
        'series, per-entry rows, flagged-row detail, the XLSX workbook and the PDF summary are unlocked after purchase.',
