...
//
// HFF — Effective Hourly Rate Report — PAID report endpoint. Returns the
// full report JSON plus the XLSX workbook and one-page PDF as base64.
//
...
//   3. DEV UNLOCK — HOURLYRATE_DEV_UNLOCK=1 (local/dev only). NOT real payment.
...
async function isEntitled(req, body) {
  const devUnlock = process.env.HOURLYRATE_DEV_UNLOCK === '1';
  ...
  return {
    ok: false,
    code: 501,
    message: 'Report generation is not configured. Set STRIPE_SECRET_KEY (buyers gate on session_id), or set HOURLYRATE_DEV_UNLOCK=1 for local testing. See README.md.',
  };
}
  ...
  try {
    ...
    return res.status(200).json({
      ...
      report: {
        ...
        clients: report.analysis.clients.map((c) => {
          const { rows, ...rest } = c;
          return rest;
        }),
        monthly: report.analysis.monthly,
        weekdays: report.analysis.weekdays,
        rates: report.analysis.rates,
        billableShare: report.analysis.billableShare,
      ...
      files: {
        xlsx: { filename: 'hourly-rate-report.xlsx', base64: xlsx.toString('base64'), bytes: xlsx.length },
        pdf: { filename: 'hourly-rate-summary.pdf', base64: pdf.toString('base64'), bytes: pdf.length },
      },
    });
  } catch (err) {
