...
//
// HCF — Email Header Audit — Stripe Checkout creator.
//
// One plan:
//   - "audit" -> $5 ONE-TIME payment (mode: 'payment')
//
// SAFETY (mirrors the proven hff-runway / hcf-phish-checker shape):
...
module.exports = async function handler(req, res) {
  ...
  if (!secretKey || secretKey.trim() === '') {
    return res.status(501).json({
      ...
      message:
        ...
        '(and STRIPE_PRICE_AUDIT) in Vercel env vars before checkout can work. See README.md.',
    });
  }
  ...
  const tier = body.tier || body.plan || 'audit';
  const TIERS = { audit: { mode: 'payment', priceEnv: 'STRIPE_PRICE_AUDIT' } };

  if (!Object.prototype.hasOwnProperty.call(TIERS, tier)) {
    return res.status(400).json({ error: 'invalid_tier', message: "Body must include a valid 'tier'. Allowed: 'audit'." });
  }
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { product: 'hcf-header-audit', tier },
    });
