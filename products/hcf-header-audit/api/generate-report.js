...
//
// HCF — Email Header Audit — full audit endpoint (ENTITLEMENT GATED).
//
...
//   3. DEV UNLOCK — HEADER_DEV_UNLOCK=1 (local/dev only). NOT real payment.
...
// with NO report attached. Paying does not buy past the guardrail.
'use strict';

const { auditHeaders } = require('../engine');
const store = require('../lib/store');

async function isEntitled(req, body) {
  const devUnlock = process.env.HEADER_DEV_UNLOCK === '1';
  ...
  if (secretKey && secretKey.trim() !== '') {
    return { ok: false, code: 402, message: 'Payment required. Buy an audit, then retry with your session_id.' };
  }
  return {
    ok: false,
    code: 501,
    message: 'Audit generation is not configured. Set STRIPE_SECRET_KEY (buyers gate on session_id), or set HEADER_DEV_UNLOCK=1 for local testing. See README.md.',
  };
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    const report = auditHeaders(headersText);
    return res.status(200).json({ ok: true, dev_unlock: !!gate.dev, report });
  } catch (err) {
    const code = err.code === 'REPORT_LINTER_FAILED' ? 422
      : (err.code === 'EMPTY_INPUT' || err.code === 'NO_HEADERS' || err.code === 'INPUT_TOO_LARGE') ? 400 : 400;
