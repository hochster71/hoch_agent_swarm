...
// has not yet supplied audio, the endpoint stays in placeholder mode (503) so
// no fake product ships.
...
// In production, resolve the buyer from the verified session, not the body.
// This stub reads a header the trusted webhook/session layer would set.
function resolveSubject(req) {
  return (req.headers && (req.headers['x-hmf-subject'] || req.headers['x-hmf-customer'])) ||
    (req.body && req.body.subject) || null;
}
...
module.exports = async function handler(req, res) {
  ...
  const requireAudio = pack.available === true; // only serve real audio for available packs

  try {
    const zip = assemblePack(pack, { subject, requireAudio });
    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Disposition', `attachment; filename="cue_pack_${pack.id}.zip"`);
    return res.status(200).send(zip);
  } catch (err) {
    if (err instanceof DeliveryDenied) {
      if (err.code === 'not_entitled') {
        return res.status(403).json({ error: 'not_entitled', message: 'Purchase/subscribe to download this pack.' });
      }
      if (err.code === 'policy_failed' && !requireAudio) {
        return res.status(503).json({
          error: 'no_audio_yet',
          message: 'This pack has no license-cleared audio yet. See product README.',
        });
      }
