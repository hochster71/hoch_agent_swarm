// engine/packager.js
//
// HMF Cue Library — pack assembly + gated delivery.
//
// assemblePack(pack, opts) returns a ZIP Buffer containing:
//   * each cue's audio file (only when real files exist and pass the policy gate)
//   * LICENSE.txt   (generated from the pack's structured license terms)
//   * manifest.json (per-cue: title, duration, bpm, key, license id)
//
// TWO gates run before any bytes are assembled:
//   1) entitlement gate (isEntitled) — buyer must have paid/subscribed.
//   2) policy gate (checkPack) — every cue must be instrumental / no-likeness,
...
// The gated entry point. opts:
//   { subject, pack, baseDir, storePath, requireAudio, nowIso }
  ...
  if (requireAudio) {
    ...
    files.push({
      ...
      data:
        ...
        'must drop license-cleared, instrumental-only tracks into cues/' + pack.id + '/\n' +
        'and re-run with requireAudio=true before selling. See products README.\n',
    });
  }
