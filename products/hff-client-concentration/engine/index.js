// engine/index.js
//
// HFF — Client Revenue Concentration Report — engine entry point.
//
// buildReport(csvText, opts) runs the whole deterministic pipeline:
//   validate -> ingest (flag bad rows) -> concentration analysis -> summarize ->
...
const { analyzeConcentration } = require('./concentration');
...
function analyze(csvText, opts) {
  ...
  const analysis = analyzeConcentration(ingested.invoices, options);
  ...
  return {
    ...
    hasPaidDates: ingested.hasPaidDates,
    summary,
    analysis,
