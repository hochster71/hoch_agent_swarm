// api/webhook.js
//
// HFF — Client Revenue Concentration Report — Stripe webhook (REAL entitlement grant).
//
...
//
// !! ORDERING BUG FIXED HERE !!
// The sibling HFF/HCF products assign `module.exports.config` BEFORE they assign
// `module.exports = handler`, which REPLACES the exports object and silently
// discards `config`. With bodyParser left enabled, Vercel would hand the handler
// a parsed body, the raw bytes would be gone, and EVERY Stripe signature
// verification would fail in production. Here `config` is attached AFTER the
// handler assignment, so it survives. There is a regression test for this in
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        await setPaid('sess:' + session.id, {
          product: 'hff-client-concentration',
