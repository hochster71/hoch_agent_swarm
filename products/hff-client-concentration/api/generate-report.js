...
//
// HFF — Client Revenue Concentration Report — PAID report endpoint. Returns the
...
//   3. DEV UNLOCK — CONCENTRATION_DEV_UNLOCK=1 (local/dev only). NOT real payment.
...
async function isEntitled(req, body) {
  const devUnlock = process.env.CONCENTRATION_DEV_UNLOCK === '1';
  ...
  return {
    ok: false,
    code: 501,
    message: 'Report generation is not configured. Set STRIPE_SECRET_KEY (buyers gate on session_id), or set CONCENTRATION_DEV_UNLOCK=1 for local testing. See README.md.',
  };
}
  ...
  try {
    ...
    return res.status(200).json({
      ...
      report: {
        ...
        paymentTiming: report.analysis.paymentTiming,
        skipped: report.skipped,
        dateOrder: report.dateOrder,
        disclaimer: report.disclaimer,
      },
      files: {
        xlsx: { filename: 'client-concentration.xlsx', base64: xlsx.toString('base64'), bytes: xlsx.length },
        pdf: { filename: 'client-concentration-summary.pdf', base64: pdf.toString('base64'), bytes: pdf.length },
      },
    });
  } catch (err) {
