...
//
// HFF — Client Revenue Concentration Report — buy-loop verification with node:test.
//
...
function clearEnv() {
  ...
  delete process.env.CONCENTRATION_DEV_UNLOCK;
}
...
test('webhook GRANTS entitlement on a signed checkout.session.completed', async () => {
  ...
  assert.strictEqual(rec.product, 'hff-client-concentration');
...
test('REGRESSION: webhook exports config with bodyParser disabled', () => {
  // The sibling HFF/HCF products assign module.exports.config BEFORE replacing
...
test('the delivered payload strips raw per-invoice rows out of each client object', async () => {
  // Keeps the response lean and avoids shipping the same rows twice.
  // (Per-invoice detail lives on the "All Invoices" sheet of the workbook.)
  clearEnv();
  const res = makeRes();
  await (load('api/generate-report.js'))(
    { method: 'POST', body: { csv: SAMPLE, session_id: 'cs_paid_1' }, headers: {} }, res
  ...
  for (const c of clients) {
    ...
    assert.ok(typeof c.sharePct === 'number', 'the aggregate fields must survive the strip');
    assert.ok(typeof c.net === 'number');
  }
});
...
test('DEV UNLOCK works locally and is labelled as dev, never as a real payment', async () => {
  clearEnv();
  process.env.CONCENTRATION_DEV_UNLOCK = '1';
  const res = makeRes();
  await (load('api/generate-report.js'))({ method: 'POST', body: { csv: SAMPLE }, headers: {} }, res);
  assert.strictEqual(res.statusCode, 200);
...
test('an entitled caller with an oversized file gets 413', async () => {
  ...
  const res = makeRes();
  await (load('api/generate-report.js'))(
    { method: 'POST', body: { csv: 'Date,Client,Amount\n' + 'x\n'.repeat(50001) }, headers: {} }, res
  );
...
test('free preview works with NO keys and NO entitlement', async () => {
  ...
  assert.ok(res.body.concentration.top1SharePct > 0);
...
test('free preview figures are a genuine SUBSET of the paid run, not a separate estimate', async () => {
  ...
  assert.strictEqual(pres.body.totals.net, gres.body.report.summary.totals.net);
  assert.strictEqual(
    pres.body.concentration.top1SharePct,
    gres.body.report.summary.concentration.top1SharePct
  );
