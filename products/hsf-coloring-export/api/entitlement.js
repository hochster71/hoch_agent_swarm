// api/entitlement.js
//
// HSF — Coloring Book Export — entitlement check (REAL).
//
// The delivery UI calls this to decide whether to show the watermark-free
// export button or send the user to checkout:
//     GET /api/entitlement?session_id=<id>
// It reads the same record the signed webhook writes via lib/store.js:
//     sess:<sessionId>   (one-time $7 purchase)
//
// Fails OPEN-SAFE: unknown / missing keys return { paid:false } with HTTP 200,
// never an error, so the UI simply stays in watermarked-preview mode.
...
module.exports = async function handler(req, res) {
  ...
  if (sessionId) {
    ...
    if (rec && rec.paid === true) { paid = true; tier = rec.tier || 'export'; }
  }
