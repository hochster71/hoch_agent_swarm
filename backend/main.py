...
def _iter_api_routes(routes):
    ...
    FastAPI >=0.13x registers `app.include_router(...)` lazily as an internal
    `_IncludedRouter` wrapper rather than flattening the child APIRoutes into
    `app.routes`. A naive `type(route).__name__ == "APIRoute"` scan therefore
    goes blind to every extracted router (see backend/routers/, main-split-plan)
    and the legacy `/api/*` -> `/api/v1/*` redirect below would 404/serve stale.
    Recursing through `original_router` keeps that redirect contract intact.
    """
    for route in routes:
...
async def has_quarantine_middleware(request: Request, call_next):
    ...
    if path.startswith("/api/") and not path.startswith("/api/v1/"):
        ...
        # Check if route is registered (including routes behind include_router)
...
# main-split-plan: /api/app-store/* + /api/v1/app-store/* extracted from this file.
