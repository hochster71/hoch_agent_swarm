...
def publish(evidence_dir: Path, instance_id: str) -> None:
    ...
    POINTER.write_text(json.dumps({
        ...
        "ledger_path": str(Path(evidence_dir) / "task_lease_ledger.jsonl"),
...
def _peak_from(ledger: Path):
    ...
    for lid, a in opened.items():
        ev.append((a, +1))
        ev.append((closed.get(lid, last), -1))
    ev.sort(key=lambda x: (x[0], x[1]))
    cur = peak = 0
    for _, d in ev:
        cur += d
        peak = max(peak, cur)
    return peak or UNKNOWN
...
def classify_active_leases() -> dict[str, Any]:
    ...
    for lock in LEASE_DIR.glob("*.lock"):
        ...
        # a lease past its expiry is NOT an active worker slot
        is_expired = False
        try:
            ...
            is_expired = False
        # soak failure-injection + recovery leases are administrative, not worker slots
...
def concurrency_truth(configured_capacity: int = 4) -> dict[str, Any]:
    ...
    if not POINTER.exists():
        return {"truth_source": "RUNTIME_SOURCE_UNPUBLISHED",
                "reason": "no daemon has declared a canonical ledger",
                "configured_capacity": configured_capacity,
                "observed_peak": UNKNOWN, "observed_at": _now()}

    ptr = json.loads(POINTER.read_text())
    ledger = Path(ptr["ledger_path"])
    api_ledger = ledger                      # the API reads the DAEMON'S declared ledger

    mismatch = None
    if not ledger.exists():
        mismatch = "RUNTIME_SOURCE_MISMATCH: declared ledger does not exist"
    ...
    out = {
        ...
        "observed_peak": peak,
        "lease_mode": "PER_TASK",
        "scheduler_instance_id": ptr.get("scheduler_instance_id", UNKNOWN),
        "ledger_path": str(ledger),
        "ledger_digest": _digest(ledger),
        "api_reads_same_ledger": str(api_ledger) == str(ledger),
    ...
    if mismatch:
        out["truth_source"] = "RUNTIME_SOURCE_MISMATCH"
